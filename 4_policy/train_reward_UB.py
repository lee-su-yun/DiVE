"""Train UB reward model on (current_map, swept_maps -> gains[29]) samples.

Loss objectives (mirrors UA train_reward.py minus subtract_cam_mean / seen_mask):

  1) loss_type=kl              KL( softmax(normalize(g)/T) || softmax(logit) )
  2) loss_type=bce_topk        BCEWithLogits with hard top-K=K_target
  3) loss_type=bce_sigmoid_z   BCEWithLogits with sigmoid((g - g.mean)/g.std/T) target

Per-sample normalization (--target_norm) applies to kl only.

Metrics (per epoch):
    val_top1, val_top3      argmax(gains) hit rate under logits
    val_p_at_5, val_p_at_10 |topk(logit, K) ∩ topk(gains, K)| / K
    val_pred_entropy        softmax-based; uniform = ln(29) ≈ 3.367
...
# support_code/ 의 model / dataset 사용
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "support_code"))
from dataset_reward_UB import get_dataloaders  # noqa: E402
from model_reward_UB import RewardNetUB  # noqa: E402
...
def parse_args():
    ...
    p.add_argument('--top_k', type=int, default=5,
                   help='K for bce_topk loss and Precision@K metric (29 pushes).')
...
def compute_loss(logit, gains, args):
...
def precision_at_k(pred_logit, gains_raw, k):
    pred_top = pred_logit.topk(k, dim=-1).indices
    gt_top = gains_raw.topk(k, dim=-1).indices
...
def train_one_epoch(model, loader, optimizer, device, scaler, args,
    ...
    for current, swept, gains in pbar:
        ...
        with torch.amp.autocast('cuda'):
            logit = model(current, swept)
            loss, _ = compute_loss(logit, gains, args)
...
def validate(model, loader, device, args):
    model.eval()
    sums = _make_sums()
    for current, swept, gains in tqdm(loader, desc='Val'):
        current = current.to(device, non_blocking=True)
        swept = swept.to(device, non_blocking=True)
        gains = gains.to(device, non_blocking=True)
        B = current.shape[0]
...
def main(rank, world_size, args):
    ...
    if rank == 0 and val_loader is not None:
        print(f'[reward_UB] train={len(train_loader.dataset)}  '
              f'val={len(val_loader.dataset)}  '
              f'k∈[{args.k_min},{args.k_max}]')

    model = RewardNetUB(base_ch=args.base_ch).to(device)
    if world_size > 1:
